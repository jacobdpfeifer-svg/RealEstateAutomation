...
6. **Partially done (code):** Outreach subsystem (§2) shipped against the seller pipeline only, per this ordering — `leads/compliance.py` (sender-identity gate, suppression list, high-risk-state hold) + `leads/outreach.py` (grounded draft rendering, `email_draft` table, `leads outreach queue|list|mark-drafted|opt-out` CLI). Every drafted/drafted-in-Gmail/opt-out event logs to the ledger with `template_version="seller_v1"`. Tests in `tests/test_outreach.py`. **Not built yet:** the actual Gmail draft creation call (done by a human/agent via Gmail MCP tools per `docs/OUTREACH_HANDOFF.md`, not by this process — it holds no Gmail credentials), the daily digest as its own report (use `leads outreach list` for now), reply-drafting, and TCPA/DNC phone screening. Buyer-side delivery stays deferred to steps 4-5 above.
